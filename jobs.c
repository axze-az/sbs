...
#  endif
	...
	if (link(filename, newname) == -1) {
		if (errno == EEXIST) {
			...
			free(newname);
			syslog(LOG_WARNING, 
			       "trying to execute job %.100s twice",
			       filename);
			return;
		} else {
	...
	if (S_ISLNK(lbuf.st_mode))
		perr("Symbolic link encountered in job %8lu (%.500s) - "
		     "aborting",
		     jobno, filename);
	...
	if ((lbuf.st_dev != buf.st_dev) || (lbuf.st_ino != buf.st_ino) ||
		...
		perr("Somebody changed files from under us for job %8lu "
		     "(%.500s) - "
		     "aborting", jobno, filename);

	if (buf.st_nlink > 2) {
		perr("Someboy is trying to run a linked script for job "
		     "%8lu (%.500s)",
		     jobno, filename);
	}
	...
	if (mailname[0] == '-')
		pabort("illegal mail name %.300s in job %8lu (%.300s)", 
		       mailname,
		       jobno, filename);

	if (nuid != uid)
		pabort("Job %8lu (%.500s) - userid %d does not match "
		       "file uid %d",
	...
	if (unlink(filename) != -1) {
		syslog(LOG_WARNING,
		       "Warning: for duplicate output file for %.100s "
		       "(dead job?)",
	       filename);
    }
	...
	*/
	...
	if (((send_mail != -1) && (buf.st_size != size)) || 
	    (send_mail == 1)) {
		PRIV_START();
		if (initgroups(pentry->pw_name, pentry->pw_gid))
			perr("Cannot delete saved userids");
...
#if defined(MAIL)
		execl(MAIL, "mail", mailname, (char *) NULL);
#else
